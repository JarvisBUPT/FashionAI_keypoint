...
if __name__ == '__main__':
    with tf.device(GPU):
        ...
        with tf.name_scope('model'):
            output = HourglassModel(params.nbStacks, params.nFeat, params.nModule, params.outDim, params.nLow, True,
                                    name='stacked_hourglass')
        print('--Model : Done')
        with tf.name_scope('loss'):
            loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=output, labels=y),
                                  name='cross_entropy_loss') * 4096 * params.nbStacks * params.batch_size
        print('--Loss : Done')
                        ...
                        for i in range(epochiter):
                            ...
                            sys.stdout.write(
                                "\r Train: {0}>".format("=" * num) + "{0}>".format(" " * (20 - num)) + "||" + str(
                                    percent)[:3] + "%" + ' time :' + str(time() - t_epoch_start)[
                                                                     :5] + 'sec  loss =' + str(avg_cost)[:6])
                            sys.stdout.flush()
                            ...
                            with tf.name_scope('batch_train'):
                                for k in range(batchSize):
                                    ...
                                    x_batch[k, :, :, :] = img / 255
                                    y_batch[:, k, :, :, :] = hm_f[:, 0, :, :, :]
                                _, c, summary = sess.run([train_rmsprop, loss, merged_summary_op],
                                ...
                                if (epoch * epochiter + i) % params.step_to_save == 0:
                                    ...
                                    summary_train.flush()
                    t_epoch_finish = time()
                    print("Epoch:", (epoch + 1), '  avg_cost= ', "{:.9f}".format(avg_cost), ' time_epoch=',
                          str(t_epoch_finish - t_epoch_start))
        t_end = time()
