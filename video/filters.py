...
class VideoFilters():
    ...
    def angleAdir(self, joints):
        ...
        links = [(0, 1), (1, 2), (2, 6), (3, 6), (4, 3), (5, 4), (10, 11), (11, 12), (12, 8), (13, 8), (14, 13),
                 (15, 14)]
    ...
    def isSayan(self, angles):
        ...
        say = True
        for i in range(10):
            if not (self.sayan_avg[i] - 1.5 * self.sayan_std[i] < angles[i] < self.sayan_avg[i] + 1.5 * self.sayan_std[
                i]):
                say = False
        return say
    ...
    def plotSayan(self, img, j):
        ...
        print('Frame')
        shape = img[int(j[9][0]) - int(np.ceil(h / 2)) + padd[0][0]:int(j[9][0]) + int(np.ceil(h / 2)) - padd[0][1],
                int(j[9][1]) - int(np.ceil(w / 2)) + padd[1][0]:int(j[9][1]) + int(np.ceil(w / 2)) - padd[1][1],
                :].shape
        print(shape)
        ...
        mask[mask != 1] = 0
        reco = img[int(j[9][0]) - int(np.ceil(h / 2)) + padd[0][0]:int(j[9][0]) + int(np.ceil(h / 2)) - padd[0][1],
               int(j[9][1]) - int(np.ceil(w / 2)) + padd[1][0]:int(j[9][1]) + int(np.ceil(w / 2)) - padd[1][1],
               :] * mask + hair
        img[int(j[9][0]) - int(np.ceil(h / 2)) + padd[0][0]:int(j[9][0]) + int(np.ceil(h / 2)) - padd[0][1],
        int(j[9][1]) - int(np.ceil(w / 2)) + padd[1][0]:int(j[9][1]) + int(np.ceil(w / 2)) - padd[1][1], :] = reco
